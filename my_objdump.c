...
void disassemble_text_section32(FILE *file, Elf32_Shdr *text, const char *section_name) {
    ...
    for (size_t i = 0; i < text->sh_size; i += 16) {
        printf("%08lx: ", (unsigned long)(text->sh_addr + i));
        for (size_t j = 0; j < 16 && (i + j) < text->sh_size; ++j) {
            printf("%02x ", code[i + j]);
        }
        printf("\n");
    }
...
void disassemble_text_section(FILE *file, Elf64_Shdr *text, const char *section_name) {
    printf("\nDesassemblage de la section: %s\n", section_name);
    unsigned char *code = malloc(text->sh_size);
    fseek(file, text->sh_offset, SEEK_SET);
    fread(code, 1, text->sh_size, file);
